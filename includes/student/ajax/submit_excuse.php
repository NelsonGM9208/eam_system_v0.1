<?php
/**
 * AJAX endpoint to submit excuse letter
 */
...
error_log("Session data before processing: " . json_encode($_SESSION));
...
try {
    ...
    error_log("Submit excuse request: " . json_encode($debug_info));
    ...
    // Check for duplicate excuse for the same event
    $duplicate_query = "SELECT excuse_id FROM excuse_letters WHERE student_id = ? AND event_id = ? AND status = 'Pending'";
    $duplicate_stmt = $con->prepare($duplicate_query);
    $duplicate_stmt->bind_param("ii", $student_id, $event_id);
    ...
    $insert_query = "INSERT INTO excuse_letters (
        student_id, event_id, excuse_type, reason, start_date, end_date, 
        is_urgent, status, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, 'Pending', NOW(), NOW())";
    
    $insert_stmt = $con->prepare($insert_query);
    $insert_stmt->bind_param("iissssi", $student_id, $event_id, $excuse_type, $reason, $start_date, $end_date, $is_urgent);
    
    if (!$insert_stmt->execute()) {
        throw new Exception('Failed to save excuse letter: ' . $insert_stmt->error);
    }
    
    $excuse_id = $con->insert_id;
    ...
    error_log("Error submitting excuse letter: " . $e->getMessage());
